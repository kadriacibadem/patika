    ...
    public List<List<Fixture<T>>> getFixtures(List<T> teams, boolean includeReverseFixtures) {
        ...
        for (int round = 0; round < totalRounds; round++) {
            ...
            for (int match = 0; match < matchesPerRound; match++) {
                ...
                fixtures.add(new Fixture<T>((Team) teams.get(home), (Team) teams.get(away)));
            }
            rounds.add(fixtures);
        }
        ...
        for (int roundNumber = 0; roundNumber < rounds.size(); roundNumber++) {
            if (roundNumber % 2 == 1) {
                Fixture<Object> fixture = (Fixture<Object>) rounds.get(roundNumber).get(0);
                rounds.get(roundNumber).set(0, (Fixture<T>) new Fixture<Object>(fixture.getAwayTeam(), fixture.getHomeTeam()));
            }
        }
